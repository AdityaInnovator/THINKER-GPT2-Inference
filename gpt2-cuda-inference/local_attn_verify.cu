...
#include "variants/attention-local-window/local_attention.cuh"
...
int main() {
    ...
    for (int example_idx = 0; example_idx < num_examples; example_idx++) {
        ...
        snprintf(input_filepath, sizeof(input_filepath), "%s/example_%i.tcin",
                 portfolio_local_attn_dir(), example_idx);
        snprintf(output_filepath, sizeof(output_filepath), "%s/example_%i.tcout",
                 portfolio_local_attn_dir(), example_idx);
        ...
        cudaCheck(cudaMalloc(&device_att, (size_t)batch_size * num_heads * seq_length * LOCAL_WINDOW_TOKENS * sizeof(float)));

        cudaCheck(cudaMemcpy(device_input, input_data, tensor_size * 3 * sizeof(float),
                            cudaMemcpyHostToDevice));

        local_attention_forward_gpu(device_output, device_qkvr, device_att,
                                    device_input, batch_size, seq_length,
