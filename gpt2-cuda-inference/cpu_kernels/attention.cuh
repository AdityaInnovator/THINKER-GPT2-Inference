...
void permute_kernel_cpu(float* q, float* k, float* v,
    ...
    // input dimensions: (B, T, 3, NH, d) in that order
    // output dimensions: 3X (B, NH, T, d) in that order
    for (int b = 0; b < B; ++b) {
...
void unpermute_kernel_cpu(float* inp, float* out, int B, int T, int NH, int d) {
    // input dimensions: 3X (B, NH, T, d) in that order
    // output dimensions: (B, T, 3, NH, d) in that order
    for (int b = 0; b < B; ++b) {
        for (int n = 0; n < T; ++n) {
            for (int nh_idx = 0; nh_idx < NH; ++nh_idx) {
                for (int d_ = 0; d_ < d; ++d_) {
...
void softmax_forward_cpu(float* out, float inv_temperature, const float* inp, int N, int T) {
    // N=B*NH, T is the sequence length
...
void attention_forward_cpu(float* out, float* qkvr, float* att,
    ...
    // att is the output of softmax, which is the attention probabilities (B, NH, T, T)
    // preatt is the input to softmax, which is the pre-attention scores (B, NH, T, T)
