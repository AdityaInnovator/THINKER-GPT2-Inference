...
__global__ void permute_kernel(float* __restrict__ q, float* __restrict__ k, float* __restrict__ v, const float* __restrict__ inp, int B, int N, int NH, int d) {
    // Implement this
...
__global__ void qk_matmul_kernel(float * __restrict__ preatt, const float * __restrict__ q, const float * __restrict__ k,
...
__global__ void pv_matmul_kernel(float * __restrict__ vaccum, const float * __restrict__ att, const float * __restrict__ v,
                                 int B, int T, int NH, int HS) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int total = B * NH * T * HS;
    if (idx >= total)
...
void attention_forward(float * __restrict__ out, float * __restrict__ qkvr, float * __restrict__ att, const float * __restrict__ inp,
                       int B, int T, int C, int NH)
{
    ...
    //    (You must have a CUDA kernel for this; CPU loops will segfault.)
    {
        ...
        int elems = B * NH * T * T;
        int blocks = (elems + threads - 1) / threads;
        qk_matmul_kernel<<<blocks, threads>>>(att, q, k, B, T, NH, HS); // att = preatt
        cudaCheck(cudaGetLastError());
    }
